...
- тестируем поведение системы - опираемся на входные и выходные данные, а не детали имплементации;
- названия тестов соответствуют проверяемому факту, а не с тем, как он исполняется в коде;
...
- не ставим [покрытие](TESTING.md#метрики-покрытия) как самоцель, достигаем принятых стандартов покрытия с помощью [эффективных unit-тестов](TESTING.md#эффективные-юнит-тесты) и эффективных интеграционных тестов.

Технологии:
- используем JUnit5 как движок для тестов;
- используем SpringBootTest для интеграционных тестов; 
- используем AssertJ для утверждений.
...
Названия тестов соответствуют тому факту, который проверяется в тесте, а не тому, как он выполняется в реализации кода.
